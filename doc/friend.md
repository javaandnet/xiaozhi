...
- `缺少目标客户端ID` - 未提供 clientid 参数
- `消息内容不能为空` - data 参数为空
- `目标客户端不存在: xxx` - 指定的客户端 ID 不存在
...
1. **客户端 ID 获取**：连接建立后，服务器会在`connection_ack`消息中返回分配的 clientId
...
1. 服务器端针对不同类型，应该进行不同处理
2. tts 需要转换成语音发送
3. sst 直接发送你文本信息，特别处理如果 client.type 为 hard（!==web）,也要发送语音发送
4. llm 经过大模型处理发送信息后，以语音模式发送到客户端
5. mcp 调用对应客户端的 mcp 能力
