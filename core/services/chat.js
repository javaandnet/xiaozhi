...
class LlmService extends BaseService {
  constructor(config = {}) {
    ...
    // Socket 客户端配置
    this.socketConfig = this.llmConfig.socket || config.socket || {};
    this.socketClient = null;
    this.useSocket = !!(this.socketConfig.url && this.socketConfig.credentials);
  ...
  async initSocketClient() {
    try {
      this.socketClient = new SocketClient(this.socketConfig.url, this.socketConfig.credentials);

      // 设置事件回调
      this.socketClient.setOnTaskComplete((data) => {
        logger.info('Socket 任务完成:', data);
      });

      this.socketClient.setOnTaskError((error) => {
        logger.error('Socket 任务错误:', error);
      });

      // 连接到服务器
      await this.socketClient.connect();
      logger.info('LLM Socket 客户端连接成功');
    } catch (error) {
      logger.error('LLM Socket 客户端初始化失败:', error.message);
      this.socketClient = null;
    }
  }
  ...
  async chatViaSocket(connectionId, message) {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        if (!resolved) {
          resolved = true;
          reject(new Error('Socket 通信超时'));
        }
      }, 30000);

      // 监听任务完成
      const onComplete = (data) => {
        if (data.questId === questId && !resolved) {
          clearTimeout(timeout);
          resolved = true;
          this.socketClient.setOnTaskComplete(null);
      ...
      const onError = (error) => {
        if (!resolved) {
          ...
          this.socketClient.setOnTaskComplete(null);
          this.socketClient.setOnTaskError(null);
          reject(new Error(error.error || error.message || 'Socket 任务失败'));
        }
      };

      // 设置回调
      this.socketClient.setOnTaskComplete(onComplete);
      this.socketClient.setOnTaskError(onError);

      // 发送消息
      this.socketClient.sendChatMessage(message, questId, { isTest: false });
      logger.info(`通过 Socket 发送消息: ${message.substring(0, 30)}...`);
    });
  }

  /**
   * 通过 HTTP API 发送消息并获取回复
   * @param {string} connectionId - 连接ID
   * @param {string} message - 用户消息
   * @returns {Promise<Object>} AI回复对象
   */
  async chatViaAPI(connectionId, message) {
    try {
      // 构建消息列表
  ...
  /**
     ...
     */
  async chat2(connectionId, message) {
    try {
      // 构建消息列表
      const messages = this.buildMessages(connectionId, message);
